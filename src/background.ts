...
async function subscribeTransfers() {
  ...
  // Sadece Ethereum adreslerini filtrele
  const ethereumAddresses = addresses.filter(isValidEthereumAddress);
  const algorandAddresses = addresses.filter(
  ...
  if (algorandAddresses.length > 0) {
    console.log(
      '[Rugsense/bg] Algorand addresses (not watched by Ethereum watcher):',
      algorandAddresses
    );
  }

  if (ethereumAddresses.length === 0) {
    console.warn('[Rugsense/bg] no Ethereum addresses to watch');
    return;
  }
